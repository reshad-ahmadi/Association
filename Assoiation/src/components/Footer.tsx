...
const Footer = () => {
  ...
  return (
    <footer className="relative bg-brand-bg text-white pt-10 sm:pt-20 pb-8 sm:pb-10 overflow-hidden border-t border-white/5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 mb-12 sm:mb-16">
          ...
          <div className="col-span-2 lg:col-span-1 space-y-4 sm:space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-gray-800 bg-gray-900/40 backdrop-blur-md">
              <span className="text-brand-primary text-[10px] sm:text-xs">✦</span>
            ...
            <div className="flex space-x-3 pt-1">
              {['X', 'Facebook', 'LinkedIn'].map((social, idx) => (
                <a key={idx} href="#" className="p-2 bg-white/5 border border-white/10 rounded-full hover:bg-brand-primary hover:text-black hover:border-brand-primary transition-all duration-300 group">
          ...
          <div className="col-span-1 space-y-4 sm:space-y-6">
            ...
            <ul className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-gray-400">
              {quickLinks.map((item) => (
                <li key={item}>
                  <a href="#" className="hover:text-brand-primary transition-colors duration-200 flex items-center group">
                    <span className="w-0 group-hover:w-2 h-[1px] bg-brand-primary mr-0 group-hover:mr-2 transition-all duration-300"></span>
                    {t(item)}
                  </a>
                </li>
              ))}
          ...
          <div className="col-span-2 lg:col-span-1 space-y-4 sm:space-y-6">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 text-white px-4 py-2.5 sm:py-3 rounded-xl focus:outline-none focus:border-brand-primary/50 focus:ring-1 focus:ring-brand-primary/50 transition-all text-xs sm:text-sm placeholder-gray-600"
              />
              <button className="absolute right-1 top-1 bg-brand-primary hover:bg-brand-secondary text-black p-2 rounded-lg transition-colors duration-200 shadow-lg scale-90 sm:scale-100">
